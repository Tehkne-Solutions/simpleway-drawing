...
export interface AlphaDomainState {
  domain: string;
  skillKey: string;
...
export interface AlphaGateSnapshot {
  status: AlphaGateStatus;
  c4Completed: boolean;
  hasCapstone: boolean;
  hasDrawingZeroRevisit: boolean;
  domains: AlphaDomainState[];
  weakest: AlphaDomainState | null;
...
export class DrizzleAlphaRepository {
  ...
  async getSnapshot(userId: string): Promise<AlphaGateSnapshot> {
    const [c4] = await this.db.select({ status: cycleProgress.status }).from(cycleProgress)
      .where(and(eq(cycleProgress.userId, userId), eq(cycleProgress.cycleKey, C4_CYCLE_KEY))).limit(1);
    const c4Completed = c4?.status === "COMPLETED";

    const states = await this.db.select().from(learnerSkillStates)
      .where(and(eq(learnerSkillStates.userId, userId), inArray(learnerSkillStates.skillKey, DOMAIN_SKILLS.map((item) => item.skillKey))));
    ...
    const [capstone] = await this.db.select({ id: artworks.id }).from(artworks)
      .where(and(eq(artworks.ownerUserId, userId), eq(artworks.type, "PROJECT"))).limit(1);
    const [revisit] = await this.db.select({ id: artworks.id }).from(artworks)
      .where(and(eq(artworks.ownerUserId, userId), eq(sql`lower(${artworks.title})`, "drawing zero revisited"))).limit(1);
    ...
    if (c4Completed && hasCapstone && hasDrawingZeroRevisit) {
      status = allCompetent ? "READY" : allDeveloping ? "READY_WITH_REVIEW" : "SUPPORT_REQUIRED";
    }
    ...
    const nextAction = !c4Completed
      ...
      : !hasDrawingZeroRevisit
        ? { title: "Refaça seu Drawing Zero", description: "Crie um STUDY com o título exato “Drawing Zero Revisited” usando a mesma referência do baseline.", href: "/create" }
        : !hasCapstone
          ? { title: "Faça o Alpha Capstone", description: "Crie um PROJECT: observe, simplifique, construa em volume e invente uma variação própria.", href: "/create" }
  ...
  async recordGateMilestone(userId: string): Promise<AlphaGateSnapshot> {
    ...
    const [existing] = await this.db.select({ id: journeyEntries.id }).from(journeyEntries)
      .where(and(eq(journeyEntries.userId, userId), eq(journeyEntries.type, "ALPHA_GATE"))).limit(1);
    if (!existing) {
      await this.db.transaction(async (tx) => {
        const [entry] = await tx.insert(journeyEntries).values({
          userId,
          type: "ALPHA_GATE",
          title: snapshot.status === "READY" ? "Foundation Alpha concluída" : "Foundation Alpha concluída com revisão",
          metadata: { status: snapshot.status, domains: snapshot.domains.map((domain) => ({ domain: domain.domain, masteryScore: domain.masteryScore, evidenceCount: domain.evidenceCount })) },
