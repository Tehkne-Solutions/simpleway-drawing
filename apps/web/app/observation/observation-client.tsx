...
function VisualPrompt({ exerciseKey }: { exerciseKey: string }) {
  if (exerciseKey.endsWith("ratio_match")) return <div className="observation-visual ratio-visual"><span className="ratio-long" /><span className="ratio-short" /></div>;
  if (exerciseKey.endsWith("angle_match")) return <div className="observation-visual angle-visual"><span /></div>;
  if (exerciseKey.endsWith("alignment_hunt")) return <div className="observation-visual alignment-visual"><i className="alignment-top" /><span className="a">A</span><span className="b">B</span><span className="c">C</span><span className="d">D</span></div>;
...
export function ObservationClient({ exercises, localUiOnly = false }: { exercises: Exercise[]; localUiOnly?: boolean }) {
  ...
  async function submit() {
    ...
    try {
      if (localUiOnly) {
        ...
        setResult({ correct: selected === activeExercise.localCorrectIndex, correctIndex: activeExercise.localCorrectIndex, explanation: activeExercise.localExplanation ?? "Compare a relação visual novamente antes da próxima tentativa.", localOnly: true });
        return;
      }
      const session = await fetch("/api/session/guest", { method: "POST" });
      if (!session.ok) throw new Error("Não foi possível iniciar sua sessão.");
      const response = await fetch("/api/observation", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ exerciseKey, answerIndex: selected, responseMs: performance.now() - startedAt.current }) });
  ...
  function next() {
    if (index + 1 >= exercises.length) setIndex(0); else setIndex((current) => current + 1);
    setSelected(null); setResult(null); setError(null); startedAt.current = performance.now();
  }
