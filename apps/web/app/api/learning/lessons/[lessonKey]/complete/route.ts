...
export async function POST(request: Request, context: { params: Promise<{ lessonKey: string }> }) {
  try {
    ...
    const lesson = getFoundationLesson(lessonKey) ?? getC2Lesson(lessonKey) ?? getC3Lesson(lessonKey);
    ...
    const result = await getLearningProgressRepository().completeLesson({ userId, lessonKey: lesson.key, lessonVersion: lesson.version, reflection: body.reflection ?? {} });
