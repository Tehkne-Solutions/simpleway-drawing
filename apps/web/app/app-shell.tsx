    ...
    <div className={`app-shell app-shell-v12 ${collapsed ? "is-collapsed" : "is-expanded"} ${mobileOpen ? "is-mobile-open" : ""} ${gameWorkspace ? "is-game-workspace" : ""}`}>
      ...
      <header className="global-header">
        <div className="global-header-left">
          <button className="shell-menu-button desktop-menu" type="button" onClick={gameWorkspace ? () => setMobileOpen((value) => !value) : toggleCollapsed} aria-label={desktopMenuLabel} aria-expanded={gameWorkspace ? mobileOpen : !collapsed}><span aria-hidden="true">☰</span></button>
          <button className="shell-menu-button mobile-menu" type="button" onClick={() => setMobileOpen((value) => !value)} aria-label="Abrir menu" aria-expanded={mobileOpen}><span aria-hidden="true">☰</span></button>
          <Link href="/" className="global-brand" aria-label="SimpleWay Drawing · Início"><span className="global-brand-mark" aria-hidden="true">S</span><span><strong>SimpleWay</strong> <b>Drawing</b></span></Link>
        </div>
      ...
      <aside className="collapsible-sidebar" aria-label="Navegação principal">
        ...
        <div className="sidebar-bottom"><Link href="/diagnostics" title={collapsed || gameWorkspace ? "Configurações" : undefined}><span className="nav-glyph" aria-hidden="true">⚙</span><b className="nav-label">Configurações</b></Link></div>
      </aside>
      {mobileOpen ? <button className="sidebar-scrim" type="button" aria-label="Fechar menu" onClick={() => setMobileOpen(false)} /> : null}
      <div className="app-stage-v12"><div id="main-content" tabIndex={-1}>{children}</div><footer className="app-footer"><span>SimpleWay Drawing</span><span>Tehkné Solutions</span></footer></div>
    </div>
  );
}
