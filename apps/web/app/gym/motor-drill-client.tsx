...
const DRILLS: Record<MotorExerciseKey, { title: string; subtitle: string; guide: string; samples: Point[] }> = {
  "exercise.swd.gym.intentional_line": {
    title: "Linha intencional",
    subtitle: "Planeje origem e destino. Execute uma linha contínua sem serrilhar.",
    guide: "M70 190 L530 90",
    samples: Array.from({ length: 41 }, (_, index) => ({ x: 70 + (460 * index) / 40, y: 190 - (100 * index) / 40 })),
  },
  "exercise.swd.gym.curve_path": {
    title: "Curve Path",
    subtitle: "Percorra a curva inteira como um único gesto contínuo.",
    guide: "M70 190 C210 40 390 245 530 90",
    samples: Array.from({ length: 61 }, (_, index) => {
      const t = index / 60;
      const mt = 1 - t;
      return {
        x: mt ** 3 * 70 + 3 * mt ** 2 * t * 210 + 3 * mt * t ** 2 * 390 + t ** 3 * 530,
        y: mt ** 3 * 190 + 3 * mt ** 2 * t * 40 + 3 * mt * t ** 2 * 245 + t ** 3 * 90,
      };
    }),
  },
  "exercise.swd.gym.ellipse_control": {
    title: "Ellipse Control",
    subtitle: "Faça uma elipse contínua acompanhando o eixo e os limites.",
    guide: "M140 140 A160 80 0 1 0 460 140 A160 80 0 1 0 140 140",
    samples: Array.from({ length: 73 }, (_, index) => {
      const angle = (Math.PI * 2 * index) / 72;
      return { x: 300 + Math.cos(angle) * 160, y: 140 + Math.sin(angle) * 80 };
    }),
  },
  "exercise.swd.gym.parallel_rails": {
    title: "Parallel Rails",
    subtitle: "Desenhe sobre a linha-alvo mantendo a direção paralela à guia superior.",
    guide: "M80 165 L520 95",
    samples: Array.from({ length: 41 }, (_, index) => ({ x: 80 + (440 * index) / 40, y: 165 - (70 * index) / 40 })),
  },
};
...
function nearestDistance(point: Point, reference: Point[]): number {
  let best = Number.POSITIVE_INFINITY;
  for (const sample of reference) best = Math.min(best, distance(point, sample));
  return best;
}
...
export function MotorDrillClient({ exerciseKey }: { exerciseKey: MotorExerciseKey }) {
  ...
  function eventPoint(event: React.PointerEvent<SVGSVGElement>): Point {
    ...
    return { x: ((event.clientX - rect.left) / rect.width) * 600, y: ((event.clientY - rect.top) / rect.height) * 280 };
  }
  ...
  async function finish(event: React.PointerEvent<SVGSVGElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/gym/motor-drill", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ exerciseKey, ...computeMetrics(finalPoints, config.samples, performance.now() - startedAt.current) }),
      });
  ...
  return (
    <div className="gym-session">
      <div className="gym-instruction">
        <p className="eyebrow">Gym · C1 Control</p>
        <h1 className="flow-title">{config.title}</h1>
        <p className="lead compact">{config.subtitle}</p>
      </div>
      ...
      <svg
        ...
        viewBox="0 0 600 280"
        role="img"
        aria-label={`Área de treino ${config.title}`}
        onPointerDown={(event) => {
          if (status === "saving") return;
          event.currentTarget.setPointerCapture(event.pointerId);
          const first = eventPoint(event);
          active.current = true;
      ...
      {result ? (
        <section className="coach-card" aria-live="polite">
          ...
          <div>
            <p className="eyebrow">Art Coach</p>
          ...
          <div className="mastery-strip">
            <span>{result.skillKey.split(".").at(-1)?.replaceAll("_", " ")}</span>
