...
function spriteMissionComplete(): boolean {
  for (const resolution of [16, 32]) {
    ...
    if (nonEmpty.length >= 3 && distinct >= 3 && colors.size >= 2 && colors.size <= 5 && saved?.onionUsed === true && saved?.previewPlayed === true) return true;
  }
  return false;
}
...
function tileMissionComplete(): boolean {
  for (const resolution of [8, 16, 32]) {
    ...
    const pixels = saved?.pixels;
    if (!Array.isArray(pixels) || pixels.length !== resolution * resolution) continue;
    ...
    if (coverage >= .12 && coverage <= .82 && colors.size >= 2 && colors.size <= 6 && edges >= 3 && Number(saved?.previewChecks ?? 0) > 0 && Number(saved?.offsetChecks ?? 0) > 0) return true;
  }
  return false;
}
...
export function PixelQuestBoard() {
  ...
  return <section className="synthesis-quest-board">
    <aside className="quest-dossier">
      ...
      <small className="quest-device-note">O progresso desta campanha é lido diretamente dos estudos salvos neste dispositivo.</small>
    </aside>
    ...
    <div className="quest-map" aria-label="Mapa da Expedição da Síntese">
      ...
      {missions.map((mission, index) => {
        ...
        return <article key={mission.id} className={`quest-node quest-node-${index + 1} ${done ? "is-complete" : ""} ${current ? "is-current" : ""}`}>
          <div className="quest-node-top"><span>{mission.number} · {mission.discipline}</span><b>{done ? "CONCLUÍDA" : current ? "MISSÃO ATIVA" : "EXPLORÁVEL"}</b></div>
