...
export function ConstructionClient({ exercises }: { exercises: Exercise[] }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const response = await fetch("/api/construction", {
        ...
        body: JSON.stringify({ exerciseKey, answerIndex: selected }),
      });
  ...
  return (
    <div className="construction-session">
      <div className="construction-head">
        <div><p className="eyebrow">Construction Lab · C3</p><h1 className="flow-title">{exercise.title}</h1></div>
      ...
      <div className="construction-options">
        {exercise.options.map((option, optionIndex) => {
          const selectedNow = selected === optionIndex;
          const className = result
            ? optionIndex === result.correctIndex ? "is-correct" : selectedNow && !result.correct ? "is-wrong" : ""
            : selectedNow ? "is-selected" : "";
          return <button key={option} type="button" className={className} disabled={Boolean(result)} onClick={() => setSelected(optionIndex)}><span>{String.fromCharCode(65 + optionIndex)}</span>{option}</button>;
        })}
      </div>
      ...
      <div className="flow-actions split-actions">
        ...
        {result ? <button className="primary" type="button" onClick={next}>{index + 1 >= exercises.length ? "Recomeçar Lab" : "Próximo exercício"}</button> : <button className="primary" type="button" disabled={selected == null || busy} onClick={submit}>{busy ? "Analisando…" : "Confirmar estrutura"}</button>}
      </div>
    </div>
  );
}
