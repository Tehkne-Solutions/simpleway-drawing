...
test("Work Chamber HTTP boundary receives only artworkId and resolves ownership before client hydration", () => {
  ...
  assert.match(page, /sem trafegar na URL/);
});
...
test("Work Chamber consumes private intent only for the same artwork version and never over an existing draft", () => {
  ...
  assert.match(canvas, /type ReviewIntent = \{ preserve: string; transform: string; baseVersionNumber: number \}/);
  assert.match(canvas, /REVIEW_INTENT_PREFIX = "swd\.create\.review-intent\.v1"/);
  assert.match(canvas, /sanitizeReviewIntent\(value: unknown, expectedVersion: number\)/);
  assert.match(canvas, /baseVersionNumber !== expectedVersion/);
  ...
  assert.match(canvas, /setNotes\(intentNotes\(privateIntent, initialArtwork\.notes \?\? ""\)\)/);
  ...
  assert.match(canvas, /a nova intenção foi descartada para não sobrescrever trabalho em andamento/);
});

test("accepted intent becomes local draft state and durable server data only through a real CANVAS save", () => {
  const canvas = source("app/create/work/work-chamber-canvas.tsx");
